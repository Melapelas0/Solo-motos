...
import { Droplet, Play, Check, Clock, ShieldCheck, HelpCircle, X, AlertCircle, Minus, Plus } from 'lucide-react';
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          ...
                          {/* Cancelar cita */}
                          {(appt.status === 'pendiente' || appt.status === 'en_progreso') && (
                            <button
                          ...
                          >
                            <HelpCircle size={12} /> Ver Detalle
                          </button>
                        </div>
                      </div>
                    </div>
    ...
    {selectedAppt && (
      <div className="modal-overlay">
        <div className="modal-content">
          ...
          <p><strong>Placa:</strong> {selectedAppt.motorcycle_plate.toUpperCase()}</p>
          <p><strong>Modelo:</strong> {selectedAppt.motorcycle_model}</p>
          <p><strong>Tipo:</strong> {selectedAppt.service_type === 'lavado' ? 'Lavado' : 'Mecánica'}</p>
          <p><strong>Fecha:</strong> {selectedAppt.date}</p>
          <p><strong>Hora:</strong> {selectedAppt.time}</p>
          <p><strong>Estado:</strong> {selectedAppt.status}</p>
          <p><strong>Descripción:</strong> {selectedAppt.description}</p>
          <div className="modal-actions" style={{ marginTop: '16px', textAlign: 'right' }}>
            <button onClick={closeDetails} className="btn btn-secondary">Cerrar</button>
          </div>
        </div>
      </div>
    )}
